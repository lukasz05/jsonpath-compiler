...
pub enum SelectionCondition {
    ...
    RuntimeSegmentCondition {
        segment_index: SegmentIndex,
    },
    Or {
        lhs: Box<SelectionCondition>,
        rhs: Box<SelectionCondition>,
    },
    And {
        lhs: Box<SelectionCondition>,
        rhs: Box<SelectionCondition>,
...
impl SelectionCondition {
    ...
    fn normalize(&self) -> SelectionCondition {
        match self {
            SelectionCondition::Or { lhs, rhs } => {
                let (lhs, rhs) = Self::normalize_and_sort_subconditions(*lhs.clone(), *rhs.clone());
                if let Some(rhs) = rhs {
    ...
    fn merge(conditions: Vec<Option<SelectionCondition>>) -> Option<SelectionCondition> {
        if conditions.is_empty() || conditions.iter().any(|condition| condition.is_none()) {
            None
        } else {
            let conditions: Vec<SelectionCondition> = conditions
                .into_iter()
            ...
            let merged_conditions = conditions
                .iter()
                .skip(1)
                .fold(conditions[0].clone(), |merged_conditions, condition| {
                    condition.merge_with(&merged_conditions, false)
                });
